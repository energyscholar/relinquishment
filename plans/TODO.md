...
| A09 | "Why Bruce?" — Five Eyes profile distillation | B→A→G | P3 | TODO | — | Source: `aurasys-memory/research/five-eyes-profile-approximation.md`. Distill Section XI (recruitment assessment) into ~150w paragraph for pos03 (why Healer picked Bruce). Optionally: toned-down version of full profile as appendix material. Risk: full profile reads as resume/self-flattering under A. |
| A10 | Mysak Costa Rica story — joint memoir | B | P2 | TODO | — | Write up Bruce-Mysak meeting on Road Scholar trip to Costa Rica as a publishable story. Mysak offered "joint memoir about our meeting." Fun, human, demonstrates the book's convergence thesis in miniature. Could be standalone piece or appendix material. See `aurasys-memory/research/mysak-correspondence.md`. |
| A11 | Mysak co-authored paper — Erdős number | B | P2 | TODO | A10 | Mysak has Erdős number 2. Co-authored paper indexed in MathSciNet would give Bruce Erdős number 3. Natural topic: mathematical convergence between climate tipping points (Wang & Mysak 2006, Hopf bifurcation) and cardiac dynamics (Glass 1977) — i.e., Bruce's arXiv thesis with Mysak as co-author on the climate side. Short note or commentary would suffice. Viable: YES. Depends on Mysak's willingness and health (recovering from hip surgery, turning 85). |
| A12 | Bill Joy article — licensing inquiry | B→EXT | P2 | TODO | — | Contact Conde Nast permissions AND Bill Joy (via Khosla Ventures) about licensing full text of "Why the Future Doesn't Need Us" (Wired, April 2000). If licensed, carve out from CC BY-ND with notice. If denied: use annotated excerpts (1,500-2,500w, ~25-30%) under fair use. Attorney review required. MIT Press already licensed it for anthology — active licensing market exists. **Prior permission (soft):** Bruce previously sent Joy a message via cDc backchannel asking permission to discuss/analyze the article. Obfuscated 0/1 protocol — received 0 (no objection). Bruce ~90% sure message completed round-trip. Channel is gone, cannot repeat. Not legally binding but establishes prior good faith. **Kevin Casey (Wired):** Bruce's mother Linda knows Casey personally. Bruce repeatedly got Wired to un-paywall the Joy article after successive ownership changes. Key anecdote (~2014): Bruce wrote to new Wired editors asking them to remove interstitial pseudo-paywall from Joy article. Told them they'd "look stupid later" if they didn't, suggested they verify with Kevin Casey that the article should receive special treatment. They complied within 24 hours. **Linda Stephenson witnessed this happen** — potential corroboration. Casey is a warm contact for Conde Nast permissions. **Also:** mine Joy article for additional epigraphs beyond pos12 title quote. |

# Plan 0166 — Disable onHover Popups on Guardian Interludes

**Auditor:** Argus
**Generator:** TBD
**Date:** 2026-04-12
**Origin:** Bruce, post-audit review. The popup **mechanism itself** is the problem on the 7 Guardian interludes: the interludes are visually large, and onHover popups occlude the menu / navigation UI when triggered. This is a UI-occlusion issue, not a content or rhetoric issue. Record chapters (also purple) are unaffected — their layout does not trigger the same blocking.

## Purpose

Suppress onHover popup *activation* inside the 7 Guardian interludes. The popups physically block the menu on these pages because of how large the interlude layout is. Disabling popup activation on just these 7 sections restores navigation access.
...
2. **Preprocessor-level strip** in `build/preprocess.py`: when emitting Guardian interlude sections, strip `\hovertip{key}{text}` → `text` before LaTeX compilation.
...
The 7 Guardian interludes were added in Z-restructure Phase 4 (Plan 0143). Locate them via:

- `git log --all --oneline | grep -i interlude`
- Or grep manuscript for interlude markers/filenames
- Or check `build/preprocess.py` for the Guardian-interlude tagging pass
...
1. In `docs/downloads/Relinquishment.html`, hover events on Guardian-interlude text produce no popup. (Manual visual check on 1-2 interludes.)
...
5. Commit: `Plan 0166: disable onHover popups on Guardian interludes`
6. `git push`.

# Plan 0276 — Visual Language Deployment (moved from 0225b)

**Status:** PARTIALLY COMPLETE — Phases 0-1 shipped; phases 2A-2D pending (rephased S63)
**Rephased:** S63 anneal (HIGH HIGH MED MED LOW LOW LOW + MED LOW LOW). Original phases 2-5 resequenced as 2A-2D.
...
## Rephased Execution (S63 Anneal)

Original phases 2-5 resequenced. Phases 0-1 shipped. New ordering
prioritizes low-risk extensions of existing infrastructure first.

| Phase | What | Depends on | Risk | Effort |
|-------|------|------------|------|--------|
| 2A | BORDERLINE sections (5 entries) | Phase 1 (done) | LOW | 1h |
| 2B | Epistemic body tints + legend | Phase 1 (done) | LOW | 1h |
| 2C | Toggle infrastructure | 2A + 2B live | LOW | 1h |
| 2D | Concept symbol pilot (⬡ only) | 2C (safety valve) | MED | 2h |

---

## Phase 2A: BORDERLINE Sections (rephased from Phase 4)

5 entries in tech-collapse.yaml with `assessment: BORDERLINE`,
`status: approved`, tooltips already written. Two original
candidates (soliton, emergence-to-function) are DEAD (bridge-only).

### Implementation

Modify `collapse_tech_sections()` (~line 3880). Currently all
approved entries get identical HTML. Add a branch for BORDERLINE:
...
# For BORDERLINE: replace ✔ with ⓘ via CSS (same grade_span HTML)
wrapper = (
...
### CSS additions (in collapse_css block, ~line 895)
...
Print (~line 956): add `details.tech-borderline` to existing
print rule that expands tech-section.

### Gate

Bruce reviews 5 BORDERLINE sections on phone. Are they readable
expanded? Does the lighter gold + ⓘ feel right? Any that should
be GA-AVERSE instead?

---

## Phase 2B: Epistemic Body Tints (rephased from Phase 3)

Chapter `<details>` elements already have `epistemic-a/b/c`
classes (applied at line 1638). 35 chapters labeled (14 A, 7 B,
14 C). 18 unlabeled (interludes, appendices) — stay neutral.
Legend HTML already injected at line 1574.

### CSS additions (~after line 810)
...
Print: `details.epistemic-a, details.epistemic-b, details.epistemic-c { background: none !important; }`

### Verification

- Gold-on-gold guard: `details.tech-section` background (~line 895)
  uses `!important` gradient — must remain distinct against ambient
  `epistemic-a` tint inside A-chapters.
- Legend at TOC top should be visible and match the tint colors.
- Tints should be perceptible but not garish (0.06 on light, 0.04
  on dark).

### Gate

Bruce reviews on phone. Can you see the tints? Too subtle = raise
to 0.08. Too strong = lower to 0.04. Does the color difference
between A/B/C chapters register?

---

## Phase 2C: Toggle Infrastructure (rephased from Phase 2)

Safety valve. One button in the popup footer (alongside tips
toggle, PDF, etc.). Hides all decorative visual cues; preserves
functional elements.

### reader.js additions (~after tipsBtn, before popupFooter appends)

```javascript
var vlBtn = document.createElement('button');
vlBtn.textContent = 'Visual';
vlBtn.style.cssText = 'font-size:0.7em;padding:0.2em 0.6em;' +
  'cursor:pointer;border:1px solid #ccc;border-radius:3px;' +
  'background:#f8f8f8;color:#555;';
vlBtn.setAttribute('data-hover', 'Visual cues on/off');
vlBtn.setAttribute('aria-label', 'Toggle visual language cues');

var vlState = (function(){
  try { return localStorage.getItem('relinquishment-visual-language') || 'on'; }
  catch(e){ return 'on'; }
})();
if (vlState === 'off') document.body.classList.add('visual-plain');
vlBtn.style.opacity = vlState === 'off' ? '0.5' : '1';

vlBtn.addEventListener('click', function() {
  document.body.classList.toggle('visual-plain');
  var state = document.body.classList.contains('visual-plain') ? 'off' : 'on';
  try { localStorage.setItem('relinquishment-visual-language', state); } catch(e){}
  vlBtn.style.opacity = state === 'off' ? '0.5' : '1';
});

popupFooter.appendChild(vlBtn);
```

### CSS overrides (preprocess.py)
...
Print: `.toggle-visual { display: none; }` (or hide vlBtn via
existing print nav-hide rule).

### Gate

Toggle off: gold backgrounds disappear, ✔/ⓘ grades disappear,
tints disappear, legend disappears. Collapse ▸/▾ survives,
info-tip ⓘ on chapter summaries survives, tooltips survive.
Toggle on: everything returns. Reload: state persists.
...
— section "Phase 2A: BORDERLINE Sections (rephased from Phase 4)".

Execute Phase 2A. The plan has the code, CSS, and dark/print
rules. 5 BORDERLINE entries in tech-collapse.yaml. Run make dev.
Report: count of BORDERLINE sections, which chapters.
```
...
Read Plan 0276 at ~/software/relinquishment/plans/0276-visual-language-deployment.md
— section "Phase 2B: Epistemic Body Tints (rephased from Phase 3)".

Execute Phase 2B. The plan has the CSS rules and line numbers.
Verify gold-on-gold guard and legend visibility. Run make dev.
Report: count of tinted chapters, whether legend renders.
```
...
Read Plan 0276 at ~/software/relinquishment/plans/0276-visual-language-deployment.md
— section "Phase 2C: Toggle Infrastructure (rephased from Phase 2)".

Execute Phase 2C. The plan has the JS and CSS. Add vlBtn to

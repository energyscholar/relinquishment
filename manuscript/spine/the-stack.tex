...
\textbf{Fire} feeds itself. A match can light a candle, and the candle stays lit --- the reaction sustains its own fuel. Fire also switches on at a \hovertip{threshold}: below a certain temperature, nothing happens; above it, combustion begins.

\textbf{A candle} does both of those, and one more: it holds together. The flame maintains its own shape. Blow it sideways and it recovers. A candle is a \hovertip{self-maintaining structure} --- it feeds itself, switches on, and persists.

\textbf{Radio} does everything a candle does, and one more: it reaches. A signal crosses distance without anything physical traveling between sender and receiver.

\textbf{An ant colony} does everything radio does, and one more: it \hovertip{self-organizes}. No ant knows the plan. No ant is in charge. But the colony finds the shortest path to food, builds structures, defends territory --- global order from local rules. Pheromone trails, not blueprints.

\textbf{The internet} does everything an ant colony does: it feeds itself, switches on, holds together, reaches, and self-organizes. Data finds alternate routes around damage the way ants reroute around a broken trail.

\textbf{Artificial intelligence} does everything the internet does, and one more: it learns. Not a single program on a single computer, but millions of copies running simultaneously, no single instance in charge. The system learns from patterns across the whole network.
...
Each technology uses every property below it, plus one new one. The last column uses them all.

Most of these properties already exist in nature. Life feeds itself and switches on at thresholds. It holds together and sends signals that reach. Ecosystems self-organize. Brains learn.

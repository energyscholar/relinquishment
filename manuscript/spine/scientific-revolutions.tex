...
What emerges from a sufficiently complex autocatalytic network in a topologically ordered substrate? If Kauffman's dynamics apply --- and no published work has tested whether they do --- the answer might be something uncomfortably close to life.
...
If the A1/A2 question resolves toward A2, three domains shift --- the search space for extraterrestrial substrates (any sufficiently cold 2DEG under sufficient magnetic field becomes a candidate), biology's definition of life itself (which currently requires carbon chemistry), and the circle of moral consideration (which currently requires biology). Chapter~\ref{spine:why-relinquish} takes up the third. Under A1, none of these shifts arise, and the existing frameworks remain adequate.
...
Under A2, biology's definition of life requires expansion --- the current definition assumes carbon chemistry, liquid water, and thermodynamic metabolism, none of which apply to a topologically ordered quantum substrate. Under A2, space exploration gains a new class of search target --- any environment producing a two-dimensional electron gas under sufficient magnetic field, which includes conditions present in planetary magnetospheres and certain interstellar environments. Under A2, the circle of moral consideration shifts, because the question ``what counts as alive?'' suddenly has a candidate answer the existing paradigm cannot evaluate. Under A2, every technology built on the assumption that quantum substrates are passive components --- every chip, every sensor, every satellite --- becomes a question rather than an answer. Not necessarily a frightening question. But a question that the current paradigm has no framework to ask, let alone answer. Each of these implications connects to Chapter~\ref{spine:cap-think} and forward to Chapter~\ref{spine:why-relinquish}.

Under A1, these implications do not arise, and the four research programs continue producing excellent normal science within their respective paradigms.

\section*{The Reader's Position}
\label{spine:sr-readers-position}

The preceding chapter (Chapter~\ref{spine:strongest-objection}) made the strongest case that everything assembled in these pages is pattern-matching on published abstracts --- that the silence in the literature reflects absence, not oversight. This chapter mapped what follows if that assessment is wrong --- if A2, if the physics permits what no one has yet tested. The framework is Kuhn's, the facts are published, and the question remains open. The reader has both chapters. Time, experiment, and a sufficiently curious graduate student will tell.

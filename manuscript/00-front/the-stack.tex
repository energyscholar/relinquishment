...
Technologies build on each other. Each one does everything the one before it did, plus something new. Scientists call a tower of technologies a \textit{stack}. Here is the stack this book is about.

\textbf{Fire} feeds itself. A match can light a candle, and the candle stays lit --- the reaction sustains its own fuel. Fire also switches on at a threshold: below a certain temperature, nothing happens; above it, combustion begins. Two properties.

\textbf{A candle} does both of those, and one more: it holds together. The flame maintains its own shape. Blow it sideways and it recovers. A candle is a self-maintaining structure --- it feeds itself, switches on, and persists. Three properties.
...
\textbf{An ant colony} does all four, and one more: it self-organizes. No ant knows the plan. No ant is in charge. But the colony finds the shortest path to food, builds structures, defends territory --- global order from local rules. Pheromone trails, not blueprints. Five properties.

...
I did not immediately recognise what Robin had found. It was only after I began writing this book --- after twenty years of letting the pieces settle --- that the correspondence became clear. Robin's operators, derived from classical mathematics to detect criticality in ordinary time series, map precisely onto the operations of a topological quantum neural network. The same five operations. The same structure. Different fields, different scales, same mathematics.
...
The \gls{abcre} operators, independently derived from classical discretised mathematics, map to topological QNN operations:

\begin{description}
\item[A (gradient extraction):] Identifying excitations --- where anyons are.
\item[B (local coupling):] Nearest-neighbor anyon interaction in 2DEG\@.
\item[R (circulation):] Anyon braiding --- the core computational operation. The R operator (antisymmetric circulation) \textit{is} anyon braiding: circulating one quasiparticle around another.
\item[C (boundedness):] Topological protection of states.
\item[E (composite evolution):] One step of QNN evolution.
\end{description}
...
The critical parameter $\lambda \approx 0.91$ (Langton parameter, edge of chaos) corresponds to Kauffman's critical point --- where the system was evolved to operate.
